...
const cityConfig = {
    ...
    "mumbai":  { slug: "mumbai",    lat: 19.0760, lng: 72.8777, name: "Mumbai, Maharashtra",   status: "DISTRESSED", fallbackAQI: 165 },
    "delhi":   { slug: "delhi",     lat: 28.7041, lng: 77.1025, name: "New Delhi, NCR",         status: "TOXIC",      fallbackAQI: 412 }
};
...
async function fetchAQI(cityKey) {
    ...
    try {
        ...
        if (data.status === 'ok' && data.data && data.data.aqi !== '-') {
            ...
            if (aqi > 300)       city.status = "TOXIC";
            else if (aqi > 100)  city.status = "DISTRESSED";
            else                 city.status = "OPTIMAL";
...
function updateCalculator() {
    ...
    if (estIntake > 800)      { resultEl.style.color = '#ef4444'; riskEl.style.color = '#ef4444'; riskEl.innerText = 'Severe'; }
    else if (estIntake > 300) { resultEl.style.color = '#f59e0b'; riskEl.style.color = '#f59e0b'; riskEl.innerText = 'High'; }
    else if (estIntake > 100) { resultEl.style.color = '#fbbf24'; riskEl.style.color = '#fbbf24'; riskEl.innerText = 'Moderate'; }
...
document.getElementById('stateSelect').addEventListener('change', (e) => {
    const valToKey = { "45": "munnar", "165": "mumbai", "412": "delhi" };
    currentCityKey = valToKey[e.target.value] || 'munnar';
